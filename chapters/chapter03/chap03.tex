...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \hl{PLACEHOLDER FOR IMAGE FROM POSTER}
        \item During the focus group sessions with students in the CS department, it was agreed upon that posting and commenting on a social network are a very desirable and easy way to validate a the authenticity of the post.It was cited that instructor intervention in the form of a comment or like could make the dissemination of learning material more reliable.
    ...
    A significant amount of development effort has been put into creating a wiki for Artemis, an official GitHub  page, relevant readme.md files for context,information about the developer and a bug/user report page. Access to these pages has been made available on every page in Artemis so that the users can see that there is a real and professional organization behind the Artemis project\cite{Fogg2002a}. Access to these features was evaluated and resultant the code was significantly re factored (discussed in detail in later chapters). Whilst default favicons were deprecated in favour of UoB's own logo \cite{UniversityofBristola} to further enhance user perception within this context, it is envisaged that if the tool is deployed in the future on UoB's in house servers this will further enhance this value.
